...
// makeBlockVolumes maps the rawblockdevices specified in the path of the container
// Experimental
func (kl *Kubelet) makeBlockVolumes(pod *v1.Pod, container *v1.Container, podVolumes kubecontainer.VolumeMap) ([]kubecontainer.DeviceInfo, error) {
	...
	for _, mount := range container.VolumeMounts {
		...
		if !ok || vol.BlockVolumeMounter == nil {
			continue
		}
		if vol.BlockVolumeMounter.GetVolumeType() == v1.PersistentVolumeBlock {
			devices = append(devices, kubecontainer.DeviceInfo{PathOnHost: vol.BlockVolumeMounter.GetVolumePath(), PathInContainer: mount.MountPath, Permissions: "mrw"})
		}
	}
	return devices, nil
}
